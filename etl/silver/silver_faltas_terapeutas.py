...
def main():
    try:
        df = read_from_bq("bronze", "faltas_terapeutas_raw")

        if df.empty:
            print("Tabela bronze vazia.")
            return

        print("Processando faltas terapeutas (Bronze)")
        ...
        df["id_falta_terapeuta"] = df["data_falta"].apply(lambda x: gerar_id("TER", x))

        cols = (
            ["id_falta_terapeuta", "data_falta", "hora_falta"]
            + [c for c in df.columns if c not in ["id_falta_terapeuta", "data_falta", "hora_falta", col_data, col_hora]]
        )

        df = df[cols]

        load_to_bq(df, "faltas_terapeutas", "silver", mode="replace")

        print("Silver: faltas_terapeutas carregado com sucesso.")
